...
export default function Home() {
  ...
  return (
    <div>
      ...
      <ExperimentalButton1 onClick={post} enableMutating>
        POST1
      </ExperimentalButton1>
      <ExperimentalButton1 onClick={post} enableMutating={["123"]}>
        POST2
      </ExperimentalButton1>
      <ExperimentalButton1 onClick={post} enableMutating={["none"]}>
        POST3
      </ExperimentalButton1>
    </div>
  );
...
function ExperimentalButton1({className, disabled, children, enableMutating, ...rest}: ComponentPropsWithoutRef<"button"> & MutatingButtonOption) {
  ...
  return (
    <MutatingButton className={classNames({loading: isMutating}, className)} disabled={isMutating ?? disabled} {...rest}>
      {children}
    </MutatingButton>
  );
}
...
function ExperimentalButton2({className, disabled, children, isMutating, ...rest}: ComponentPropsWithoutRef<"button"> & {isMutating: boolean}) {
  return (
    <button className={classNames({loading: isMutating}, className)} disabled={isMutating ?? disabled} {...rest}>
      {children}
    </button>
  );
}
