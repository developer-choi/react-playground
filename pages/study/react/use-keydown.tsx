...
export default function WindowKeyDownPage({videoUrl}: PageProp) {
  ...
  useKeyDown(
    FULLSCREEN_TOGGLE_KEYBOARD_EVENT,
    useCallback(() => {
      if (videoRef.current) {
        const videoElement = videoRef.current;
  ...
  return (
    <Wrap>
      ...
      <p>슬래시를 누르면, 입력박스로 포커스가 이동합니다. (구글 검색페이지 따라하기)</p>
...
export const getServerSideProps: GetServerSideProps<PageProp> = async (context) => {
  const {
    video: {url}
  } = await getVideoOneApi(1, context);
