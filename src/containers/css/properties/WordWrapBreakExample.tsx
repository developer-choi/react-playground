...
export default function WordWrapBreakExample() {
    ...
    return (
        <div className="WordWrapBreakExample-wrap">
            ...
            <ul className="list-style-inside">
                ...
                <li>심지어 word-wrap의 word-break속성을 해도 단어단위로 줄내림되지 않음.</li>
            </ul>
...
function KoreanSample({className, title}: {className: string, title: string}) {
    return (
        <div className={`sample ${className}`}>
            ...
            소녀시대 내에서 악기 연주에 가장 능숙한 멤버다. 플룻과 피아노를 전공하시고, 과거 피아노 학원을 운영하셨던 어머니 덕분에 어렸을 때부터 쉽게 피아노를 접하고 배울 기회가 많았으며, 그 결과 상당한 연주 실력을 갖추고 있다. 데뷔 후에는 방송, 콘서트 등에서 피아노 연주를 하는 모습을 가끔씩 선보였다.
...
function EnglishSample({className, title}: {className: string, title: string}) {
    return (
        <div className={`sample ${className}`}>
            ...
            He is the most proficient member of musical instruments in Girls' Generation. Thanks to my mother who majored in flute and piano and ran a piano academy in the past, I had many chances to learn and play the piano easily from a young age, and as a result, I have considerable performance skills. After his debut, he occasionally showed playing piano at broadcasts and concerts.
