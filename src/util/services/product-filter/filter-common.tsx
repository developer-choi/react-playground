...
import type {FilterFormData, FilterPkListInQueryString, FilterPkOriginalRecord, AppliedFilter, PriceFilterValue, ProductListPageParam, RegularFilterType} from "@type/services/filter";
...
export function ProductListPageParamProvider({children, value}: PropsWithChildren<{value: ProductListPageParam}>) {
  return <ProductListPageParamContext.Provider value={value}>{children}</ProductListPageParamContext.Provider>;
}
...
function filterListResponseToPkOriginalRecord(response: FilterListResponse): FilterPkOriginalRecord {
  ...
  const {category, color, brand, size} = Object.entries(responseByFilterType).reduce(
    (a, [filterType, filterList]) => {
