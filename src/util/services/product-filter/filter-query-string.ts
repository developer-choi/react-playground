...
import type {FilterFormData, FilterPkListInQueryString, AppliedFilter, FilterType, PriceFilterType, PriceFilterValue, RegularFilterPkList, RegularFilterType} from "@type/services/filter";
...
export function useFilterQueryString() {
  ...
  const applyFilterInQueryString = useCallback(
    (data: FilterPkListInQueryString | FilterFormData) => {
      const {["min-price"]: minPrice, ["max-price"]: maxPrice, ...regularFilter} = data;

      const stringifiedRecord = Object.entries(regularFilter).reduce(
        (a, [filterType, pkList]) => {
  ...
  const removeFilterInQueryString = useCallback(
...
export function validateFilterQueryString(query: ParsedUrlQuery) {
  ...
  const regularFilterResult: RegularFilterPkList<number> = Object.entries(regularFilterQuery)
    .map(([filterType, queryString]) => {
