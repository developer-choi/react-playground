...
export function dateToMonthDetail(targetDate = new Date()): MonthDetail {
  ...
  return {
    lastDateOfTheMonth: lastDate,
    firstDateOfTheMonth: firstDate
  };
}
...
export function getCalendarDates(targetDate = new Date()): Date[] {
  ...
  const prevMonthDates = new Array(prevMonthDayCount).fill("").map((value, index) => new Date(year, month, index - prevMonthDayCount));

  const currentMonthDates = new Array(lastDateOfTheMonth.getDate()).fill("").map((value, index) => new Date(year, month, index + 1));

  const nextMonthDayCount = 7 - lastDateOfTheMonth.getDay() - 1;

  const nextMonthDates = new Array(nextMonthDayCount).fill("").map((value, index) => new Date(year, month + 1, index + 1));
