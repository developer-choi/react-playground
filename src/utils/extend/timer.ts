...
export interface PeriodTimerParam {
  ...
  format?: typeof defaultPeriodTimerFormat;
}

export interface PeriodTimerResult {
  isProceeding: boolean;
  dateText: '' | string; // 진행중이 아니면 빈문자열, ex: '2일 12 : 37 : 00'
}
 ...
 *
 */
export function usePeriodTimer({period, format = defaultPeriodTimerFormat}: PeriodTimerParam): PeriodTimerResult {
  const currentTimestamp = Date.now();
  const enabled = !period
    ? false
    : currentTimestamp > period.startTimestamp && currentTimestamp < period.endTimestamp;
  ...
  const result = (!enabled || status !== 'proceeding' || !period) ? null : calculateRemainTime(snapshotTimestamp, period.endTimestamp);

  return {
    isProceeding: status === 'proceeding',
    dateText: !result ? '' :  format(result)
  };
}
...
function defaultPeriodTimerFormat({days, hours, minutes, seconds}: ReturnType<typeof calculateRemainTime>) {
