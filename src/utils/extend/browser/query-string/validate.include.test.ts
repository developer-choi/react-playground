  ...
  describe('Edge cases', () => {
    ...
    describe('When the underlying validateString fails', () => {
      it('should propagate the result from validateString', () => {
        expect(() => validateIncludeString(undefined, [...ALLOW_LIST], {required: true, throwable: true})).toThrow(ValidationError);
    ...
    describe('When the queryValue is a valid string but not in the allow-list', () => {
      it('should throw a ValidateError when throwable is true', () => {
        expect(() => validateIncludeString(INVALID_VALUE, [...ALLOW_LIST], {throwable: true})).toThrow(ValidationError);
      });
    ...
    describe('When the allow-list itself is empty', () => {
      it('should throw a ValidateError when throwable is true', () => {
        expect(() => validateIncludeString(VALID_VALUE, [], {throwable: true})).toThrow(ValidationError);
      });
