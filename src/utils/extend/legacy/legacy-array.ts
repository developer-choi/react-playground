...
export function splitIntoPieces<T>(array: T[], splitLength: number): T[][] {
  ...
  return new Array(splitCount).fill('').map((_, index) => array.slice(index * splitLength, (index + 1) * splitLength));
}
...
export function splitEvenly<T>(array: T[], maxSize: number): T[] {
  ...
  const overCount = arrayLength - maxSize + 1;
  const magic = arrayLength / overCount;
