...
export function getTestStatus401ClientApi() {
  return fetchFromClient("/api/random/status-401", {authPolicy: 'private', method: 'GET'});
}

export async function getTestPlainListApi(page: number) {
  const {data} = await customFetchOnBothSide<PlainListApiResponse>('/api/random/plain-list', {
    method: 'GET',
    query: {
      page
    }
